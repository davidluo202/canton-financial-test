  ...
  return (
    <>
      ...
      >
        ...
        <button
          onClick={handleClick}
          className="bg-blue-600 hover:bg-blue-700 text-white px-2 py-3 text-sm font-medium transition-all duration-200 animate-pulse-subtle shadow-md"
          style={{ 
        ...
        {/* Desktop Popover (Normal State) */}
        {!isMobile && isOpen && !isExpanded && (
          <div
            className="absolute left-full ml-2 top-1/2 -translate-y-1/2 w-[800px] h-[450px] backdrop-blur-xl bg-white/95 shadow-2xl rounded-lg p-6 overflow-y-auto transition-all duration-300 ease-in-out border border-gray-200"
      ...
      {/* Desktop Modal (Expanded State) - Rendered via Portal to avoid CSS transform clipping */}
      {!isMobile && isOpen && isExpanded && typeof document !== 'undefined' && createPortal(
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm transition-all duration-300">
          <div
            className="w-[90vw] h-[90vh] bg-white shadow-2xl rounded-xl p-8 overflow-y-auto border border-gray-200"
